...
Brutal.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      Brutal.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = Brutal.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Brutal Bot Inc.`, "footerText": "*_Dragon Dynasty_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`Dragon Dynasty`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nDragon Dynasty. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    //auto voice note by Brutal
    for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					Brutal.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
					}
			}
			//auto sticker
			for (let anji of setik){
				if (budy === anji){
					...
					Brutal.sendMessage(from, result, sticker, { quoted: mek})
					}
			}
			//auto image
			for (let anjh of imagi){
				if (budy === anjh){
					...
					Brutal.sendMessage(from, result, image, { quoted: mek})
					}
			}
			// auto text 
...
*Thank you for registering in the Dragon Dynasty Database*
...
*「 Dragon Dynasty 」*`
	    ...
	    // console.log(e)
			...
			reply(`*Thank you for being registered Dragon Dynasty*`)
		}, 2000)
        break
...
break
case 'h':
reply('han bhai/bahen woh nikal gaya.. chinta na karo usse dusre jagah bhi sukun nasib nahi hoga 😂😂')
break
case 'okeyx':
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `Dragon Dynasty`, [
          {
            buttonId: `command`,
            buttonText: {
...
case 'script':
		case 'sc':
		case 'sourcecode':
		Brutal.sendMessage(from, { text: "https://github.com/XTG-BRUTAL/dragon-dynasty", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
nm = body.slice(7)
					...
					Brutal.sendMessage(from, `Success, please check with *${prefix}vnlist*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Brutal.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
					// add sticker
					case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					Brutal.sendMessage(from, `Success, please check with *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
					case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Brutal.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					// add image
					case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					Brutal.sendMessage(from, `Success, please check with *${prefix}imagelist*`, MessageType.text, { quoted: mek })
					break
					case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Brutal.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
					// add respond
					case 'addrespond':
			if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
...
case 'linkgroup':
              ...
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
                   if (!isBotGroupAdmins) return reply('bot not admin')
					linkgc = await Brutal.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\n${groupName} *Group Link*`
					Brutal.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
                  case 'resetgrouplink':
         ...
         Brutal.query({json, expect200: true})
...
await Brutal.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply('this feature is only for group')
     ...
     Brutal.sendMessage(from, buffer, image, {quoted: mek, caption: 'Here'})
     break
             case "bc":  
        case "bcimage":  
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					buff = await Brutal.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						Brutal.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *Dragon Dynasty BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					Brutal.sendMessage(_.jid, `*Dragon Dynasty BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
        ...
        case 'tiktoknowm':   case 'tiktok':
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											me = `*Link* : ${a.data}`
											Brutal.sendMessage(from,meno,MessageType.video,{mimetype:'video/mp4',quoted: mek, caption:me})
											})
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											Brutal.sendMessage(from,audin,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
											})
										}).catch((err) => reply(`Invalid link`))
									break
					// ml hero
...
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              ...
              buttonsMessage = {footerText:'Dragon Dynasty', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, `Dragon Dynasty`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             Brutal.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 Dragon Dynasty BROADCASTING 」*\n\n${body.slice(4)}`})
}
...
Link : ${get_resultP.url_audio}
            `
            Brutal.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
            ...
            Brutal.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Brutal`)
